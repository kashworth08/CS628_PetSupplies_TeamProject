  ...
  async up(db) {
    ...
    try {
      ...
      if (collections.length === 0) {
        await db.createCollection("reviews"); // Use the db object directly
        console.log("Review collection created.");
      } else {
        console.log("Review collection already exists.");
      }

      const Review = mongoose.model("Review", reviewSchema); // Define your Mongoose model
